...
local physics = require("physics")
  ...
  timer.performWithDelay( 10,increaseProgressView,50)
...
local background = display.newImage('Imagens/bgplay4.png', largura, bgHeight)
...
--Runtime:addEventListener("touch", toque)
     ...
     local tempo = display.newImage("imagens/tempo.png")
      tempo.x = 20
      tempo.y = 22 
      tempo.height= 50
      tempo.width=30
...
--   createbloco( 800, 1010)
-- createbloco( 880, 1400)
--  createbloco( 900, 2000 )
--  createbloco( 1000, 1500)
--  createbloco( 400, 300)
--  createbloco( 120, 680)
-- createbloco( 360, 700)
--  createbloco( 760, 500 )
...
function addtime()
    time = display.newImage('imagens/ponto.png')
...
function addUp()
    up = display.newImage('imagens/up.png')
    up.name = 'up'
     up.width = 50
    up.height = 50
    local blocoRandom=math.random(4) 
  ...
  --   if(event.phase == "began")then
        if((event.object1.myName=="block" and event.object2.myName=="pinguo")
         or(event.object1.myName=="pinguo" and event.object2.myName=="block"))then
          ...
          ---  event.object1.myName=nil
            Ax = Ax + 5
           -- bloco:transition.to(blocos, {y=Ax, time=2000})
            print(Ax)
           transition.to(blocos, {y=Ax, time=200}) 
           -- transition.to(worldGroup, {y=Ax, time=200}) 
